...
export async function sendEmail(message: EmailMessage): Promise<void> {
  ...
  const from =
    process.env.EMAIL_FROM?.trim() || "ZZP Schakel <onboarding@resend.dev>";
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden;">
        ...
        <tr><td style="padding:18px 28px;background:#f8fafc;border-top:1px solid #e2e8f0;">
          <p style="margin:0;font-size:12px;color:#94a3b8;">Je ontvangt deze e-mail omdat je een account hebt bij ZZP Schakel. Voorkeuren aanpassen kan in je instellingen.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
