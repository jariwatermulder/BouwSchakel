...
De huidige repository bevat een **statische HTML/CSS/JS-website voor "JW Hout & Tuinbouw"** (een hoveniersbedrijf) — geen build-tool, geen framework, geen `package.json`. Deze site is functioneel losstaand van ZZP Schakel.

**Aanbeveling:** ZZP Schakel wordt gebouwd als een nieuwe Next.js-applicatie in een subdirectory (`/app` op repo-niveau, of `/bouwschakel`) zodat de bestaande statische site niet breekt. De keuze tussen (a) ZZP Schakel náást de bestaande site of (b) ZZP Schakel als vervanging is een **beslissing die vóór FASE 1 door de opdrachtgever wordt bevestigd**. Tot die bevestiging blijft de bestaande site ongemoeid.
